...
func (r *PositionRepository) Create(ctx context.Context, pos *models.Position) error {
	query := `
		INSERT INTO positions (car_id, drive_id, latitude, longitude, heading, speed, power, odometer, battery_level, range_km, inside_temp, outside_temp, elevation, recorded_at)
...
func (r *PositionRepository) GetLatestByCarID(ctx context.Context, carID int64) (*models.Position, error) {
	query := `
		SELECT id, car_id, drive_id, latitude, longitude, heading, speed, power, odometer, battery_level, range_km, inside_temp, outside_temp, elevation, recorded_at
